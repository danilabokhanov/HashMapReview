...
State::State(IterationCount max_iters, const std::vector<int64_t>& ranges, int thread_i,
             int n_threads, internal::ThreadTimer* timer, internal::ThreadManager* manager)
    : total_iterations_(0),
      batch_leftover_(0),
...
std::unique_ptr<BenchmarkReporter> CreateReporter(std::string const& name,
...
size_t RunSpecifiedBenchmarks(BenchmarkReporter* display_reporter,
    ...
    if (!display_reporter) {
        default_display_reporter =
            internal::CreateReporter(FLAGS_benchmark_format, internal::GetOutputOptions());
    ...
    if (!fname.empty()) {
        ...
        if (!file_reporter) {
            default_file_reporter =
                internal::CreateReporter(FLAGS_benchmark_out_format, ConsoleReporter::OO_None);
